...
#   +===================================================+
    ...
    def partition_list(self, x):       
        ...
        #true when temp is none, return 
        if not temp:  
            return self.head
        
        #initialize less than list & greater than list 
        #with dummy value 
        l_list = LinkedList(temp)
        g_list = LinkedList(temp)

        l_list.make_empty()
        g_list.make_empty()

        l_list_endptr = g_list_endptr = None
        while temp is not None: 
            if temp.value < x: 
                l_list.append(temp.value)
                if l_list.length == 1: 
                    l_list_endptr = l_list.head

                else: 
                    l_list_endptr = l_list_endptr.next


            if temp.value >=x: 
                g_list.append(temp.value)
                if g_list.length == 1: 
                    g_list_endptr = g_list.head

                else: 
                    g_list_endptr = g_list_endptr.next 

            temp = temp.next

        #print("less than list head: ", l_list.head.value)
        #print("less than list end ptr: ", l_list_endptr.value)
        #print("greater than list head: ", g_list.head.value)
        #print("greater than list end ptr: ", g_list_endptr.value)
        
        if l_list_endptr is None:
            return g_list
          
        l_list_endptr.next = g_list.head
        self.head = l_list.head
        return self.head
