...
def is_balanced_parentheses(test_string):
    ...
    #unbalanced if it starts a ) and/or ends with a (
    ...
    for each in range(len(test_string)): 
        if test_string[each] == "(": 
            test_Stack.push(test_string[each])

        elif len(test_Stack.stack_list) > 0 and test_string[each] == ")": 
            test_Stack.pop()

        else: 
            return False 

    if len(test_Stack.stack_list) > 0: 
        return False 
